...
class SocketSender(MessageSender):
    ...
    def initSender(self):
        ...
        if self.dgram:
                self.sock.send(bytes(''.encode('utf8')))
    ...
    def sendMessage(self, msg):
        ...
        if self.send_by_bytes:
            for c in line:
                try:
                    if self.ssl:
                        self.sock.write(bytes(c.encode('utf8')))
                    else:
                        self.sock.send(bytes(c.encode('utf8')))
                except error as e:
            ...
            while repeat:
                try:
                    ...
                    if self.ssl:
                        self.sock.write(bytes(line.encode('utf8')))
                    else:
                        self.sock.send(bytes(line.encode('utf8')))
