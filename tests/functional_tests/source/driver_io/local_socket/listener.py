...
class LocalSocketListener(object):
    ...
    def listen_on_socket_stream(self, driver_name, message_counter, server_address, raw=False):
        ...
        self.log_writer.info("SUBSTEP Content received stream\n>>>Content: [%s]\n" % (merged_incoming_data))
        if raw:
            return merged_incoming_data
        return merged_incoming_data.splitlines(keepends=True)

    def listen_on_socket_dgram(self, driver_name, message_counter, server_address, raw=False):
        received_buffer_size = 8192
        message_arrived = False
        merged_incoming_data = ""
        generated_socket = self.local_socket_common.generate_socket_for_type(driver_name)
        ...
        self.log_writer.info("SUBSTEP Content received dgram\n>>>Content: [%s]\n" % (merged_incoming_data))
        if raw:
            return merged_incoming_data
        return merged_incoming_data.splitlines(keepends=True)
