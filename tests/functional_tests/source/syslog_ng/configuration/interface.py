...
class SyslogNgConfigInterface(object):
    def __init__(self, testdb_logger, testdb_config_context, driver_data_provider, file_register, file_writer, topology):
        ...
        self.config_writer = SyslogNgConfigWriter(testdb_logger=testdb_logger, syslog_ng_config=self.syslog_ng_config)
    ...
    def write_config(self):
        return self.config_writer.write_config(file_register=self.file_register, file_writer=self.file_writer, topology=self.topology)

    def write_raw_config(self, raw_config):
        return self.config_writer.write_raw_config(raw_config=raw_config, file_register=self.file_register, file_writer=self.file_writer, topology=self.topology)
    ...
    def generate_config(self, use_internal_source=True):
        ...
        self.config_writer.write_config(file_register=self.file_register, file_writer=self.file_writer, topology=self.topology)

    def generate_raw_config(self, raw_config):
        self.config_writer.write_raw_config(raw_config=raw_config, file_register=self.file_register, file_writer=self.file_writer, topology=self.topology)
