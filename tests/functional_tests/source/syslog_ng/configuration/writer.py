class SyslogNgConfigWriter(object):
    ...
    def write_raw_config(raw_config, file_register, file_writer, topology):
        syslog_ng_config_path = file_register.get_registered_file_path(prefix="config_%s" % topology, extension="conf")
        file_writer.write_content_to_regular_file(file_path=syslog_ng_config_path, content=raw_config)

    def write_config(self, file_register, file_writer, topology):
