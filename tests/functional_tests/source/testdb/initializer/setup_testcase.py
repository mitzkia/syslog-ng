...
class SetupTestcase(SetupClasses):
    ...
    def run(self, expected_raw_message=None, message_counter=1):
        ...
        if self.source_driver in ['pipe', 'tcp', 'udp', 'syslog', 'network']:
            self.send_message(message_counter=message_counter)
        self.stop_listeners()
    ...
    def build_config_with_destination_drivers(self, destination_drivers="all", source_drivers="file"):
    ...
    def start_listeners(self):
        self.threaded_listener.start_listeners()
