...
def test_config_change_after_start(tc):
    ...
    bsd_message2 = tc.new_bsd_message("message for modified source-1").build()
    file_source.write_message(bsd_message2)

    output_message = file_destination.read_all_messages()
    expected_output_message = bsd_message2.remove_priority().build()
    assert output_message == expected_output_message.get_message()
