...
def test_syslog_parser(tc):
    config = tc.new_config()
    config.add_global_options({"stats_level": 3})
    file_source = config.get_file_source({"file_path": "input", "flags": "no-parse"})
    syslog_parser = config.get_syslog_parser({"flags": "syslog-protocol"})
    file_destination = config.get_file_destination({"file_path": "output"})
    config.create_logpath(sources=[file_source], parsers=[syslog_parser], destinations=[file_destination])

    test_message = tc.new_ietf_message().build()
