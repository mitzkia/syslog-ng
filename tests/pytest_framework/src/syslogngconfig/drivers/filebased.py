...
class FileBasedFactory(Drivers):
    ...
    def generate_driver_options(self, file_name_prefix, driver_options):
        self.mandatory_option = self.generate_mandatory_option({self.mandatory_option: file_name_prefix}, driver_type=self.driver_type)
...
class FileDestination(FileBasedFactory):
    ...
    def get_expected_output_message(self, message_parts, expected_message_counter=1):
        ...
        return self.message.create_multiple_bsd_messages(defined_bsd_message_parts=message_parts, message_counter=expected_message_counter)

    def read(self, expected_message_counter=1):
        return self.driver_io.read_content(self.driver_connection_path, expected_message_counter=expected_message_counter)
