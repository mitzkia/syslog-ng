...
# Low level functions
    ...
    def get_syslog_ng_start_command(self, syntax_check=None):
        start_command = "{} ".format(self.syslog_ng_binary)
        start_command += "-Fedtv --no-caps --enable-core "
        start_command += "-f {} ".format(self.syslog_ng_runtime_files['config_path'])
        start_command += "-R {} ".format(self.syslog_ng_runtime_files['persist_path'])
        start_command += "-p {} ".format(self.syslog_ng_runtime_files['pid_path'])
        start_command += "-c {} ".format(self.syslog_ng_runtime_files['control_socket_path'])
        if syntax_check:
    ...
    def wait_for_console_messages(self, messages, expected_occurance=1):
        std_error_file = FileWaitForEvent(self.logger_factory, self.syslog_ng_runtime_files['stderr_path'])
        result = []
    ...
    def is_core_file_exist(self):
        for syslog_ng_core_file in [os.path.join(os.getcwd(), "core*")]:
            if File(self.logger_factory, syslog_ng_core_file).is_wildcard_file_exist():
                core_backtrace_path = os.path.join(self.syslogngpath.working_dir, "core_backtrace.txt")
                gdb_command = "gdb --core {} {} -ex 'bt full' --batch".format(syslog_ng_core_file, self.syslog_ng_binary)
                ...
                File(self.logger_factory, syslog_ng_core_file).move_file("{}/".format(self.syslogngpath.working_dir))
                self.syslog_ng_process = None
                self.syslog_ng_pid = None
