...
@pytest.mark.parametrize(
    "prefix, extension, subdir, expected_result",
    [
...
@pytest.mark.parametrize(
    "prefix, expected_result", [("test_prefix", "/tmp/test_prefix_{}.log".format(UNIQUE_ID))]  # default extension
)
...
def test_get_registered_file_path_key_already_registered(tc_unittest):
    file_register = FileRegister(tc_unittest.fake_logger_factory(), working_dir="/tmp")
    assert (
        file_register.get_registered_file_path(prefix="test_prefix", extension="txt")
        == "/tmp/test_prefix_{}.txt".format(UNIQUE_ID)
    )
    assert (
        file_register.get_registered_file_path(prefix="test_prefix", extension="txt")
        == "/tmp/test_prefix_{}.txt".format(UNIQUE_ID)
...
def test_get_registered_file_path_same_key_different_extenstion(tc_unittest):
    ...
    assert (
        file_register.get_registered_file_path(prefix="test_prefix", extension="log")
        == "/tmp/test_prefix_{}.log".format(UNIQUE_ID2)
    )
    assert (
        file_register.registered_files
        == {
