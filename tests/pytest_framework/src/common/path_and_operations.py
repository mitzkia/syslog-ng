...
def open_pipe(file_path, flags):
    return os.open(str(file_path), flags)

def move_file(src_file_path, dst_dir):
    # Python 2/3 compatibility note: shutil.move() can work only with string representation of path
    shutil.move(str(src_file_path), dst_dir)
