...
class PipeSource(SourceDriver):
    def __init__(self, syslog_ng_config, logger_factory, instance_parameters, build_status, options):
        ...
        super(PipeSource, self).__init__(logger_factory, instance_parameters, self.pipe_io_ref, empty_driver_content)
        # set mandatory_options
        self.working_dir = instance_parameters.get_working_dir()
        options = self.set_file_path_mandatory_option(options, "pipe_source", self.working_dir)
        # build statement
        self.build_statement(build_status, syslog_ng_config, "sources", options)

    def update_options(self, options):
        options = self.update_file_path_mandatory_option(options, "pipe_source", self.working_dir)
