...
class FileDestination(DestinationDriver):
    def __init__(self, syslog_ng_config, logger_factory, instance_parameters, build_status, options):
        ...
        super(FileDestination, self).__init__(logger_factory, instance_parameters, self.file_io_ref, empty_driver_content)
        # set mandatory_options
        self.working_dir = instance_parameters.get_working_dir()
        options = self.set_file_path_mandatory_option(options, "file_destination", self.working_dir)
        # build statement
        self.build_statement(build_status, syslog_ng_config, "destinations", options)

    def update_options(self, options):
        options = self.update_file_path_mandatory_option(options, "file_destination", self.working_dir)
