...
class CommandExecutor(object):
    ...
    def run(self, command, stdout_path, stderr_path):
        ...
        self.logger.debug("The following command will be executed\
        \n->Command:[{}]".format(printable_command))
        cmd = psutil.Popen(
            executable_command,
            stdout=stdout.open_file(mode="w"),
            stderr=stderr.open_file(mode="w")
        )
        exit_code = cmd.wait(timeout=self.START_TIMEOUT)

        stdout_content, stderr_content = self.process_std_outputs(stdout, stderr)
        self.process_exit_code(printable_command, exit_code)
        return {
            "exit_code": exit_code,
    ...
    def process_std_outputs(self, stdout, stderr):
        stdout_content = stdout.open_file("r").read()
        stderr_content = stderr.open_file("r").read()
        self.logger.debug("Stdout:[{}]".format(stdout_content))
        self.logger.debug("Stderr:[{}]".format(stderr_content))
        return stdout_content, stderr_content

    def process_exit_code(self, command, exit_code):
        exit_code_debug_log = "\n->Command:[{}]\
        \n->Exit code:[{}]".format(command, exit_code)
        if exit_code == 0:
