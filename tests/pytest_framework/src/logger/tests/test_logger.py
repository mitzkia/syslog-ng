...
def test_write_message_based_on_value(tc_unittest):
    temp_file = tc_unittest.get_temp_file()
    logger = Logger("UnitTest", temp_file, logging.DEBUG)
    logger.print_message_based_on_value("TestMessage", False)
    assert " - UnitTest - ERROR - TestMessage: [False]" in temp_file.read_text()

    logger.print_message_based_on_value("TestMessage", True)
    assert " - UnitTest - DEBUG - TestMessage: [True]" in temp_file.read_text()
