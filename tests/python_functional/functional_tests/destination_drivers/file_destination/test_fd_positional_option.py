...
def test_fd_positional_option_missing(config, syslog_ng):
    ...
    file_destination = config.create_file_destination(file_name=None)
...
def test_fd_positional_option_empty(config, syslog_ng, generated_bsd_messages):
    file_source = config.create_file_source(file_name="input.log")
...
def test_fd_positional_option_custom_dir(config, syslog_ng, generated_bsd_messages):
